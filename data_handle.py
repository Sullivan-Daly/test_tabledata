...
class CElastic(CData):
    def __init__(self, t_config, t_data):
        ...
        self.xLastResult = 0
        self.nIndexSize = 0
        self.tData.nok_id = self.get_nok_id()
        self.tData.ok_id = self.get_ok_id()
    ...
    def get_tweets(self, nFrom=0, nSize=2200):
        nIndexSize = int(self.xEs.count(index=self.sIndexName,
                                             body={"query": {"match":{self.sTriName:{"query": 0}}}})['count'])

        l_fields = ['timestamp_ms', 'id_str', 'text', 'user.id_str', 'has_image']

        x_response = self.xEs.search(index=self.sIndexName, doc_type=self.sDocTypeName,
                                     _source=l_fields, stored_fields=l_fields, sort=['timestamp_ms:asc'],
                                     from_=nFrom, size=3*nSize, body={"query":{"match":{self.sTriName:{"query": 0}}}})

        data = self._return_from_scroll(x_response, nSize)
        data['recordsTotal'] = nIndexSize
        data['recordsFiltered'] = nIndexSize
        return data
    ...
    def get_ok(self, nFrom = 0, nSize = 2200):
        nIndexSize = int(self.xEs.count(index=self.sIndexName,
                                        body={"query": {"match": {self.sTriName: {"query": 1}}}})['count'])

        l_fields = ['timestamp_ms', 'id_str', 'text', 'user.id_str', 'has_image']

        x_response = self.xEs.search(index=self.sIndexName, doc_type=self.sDocTypeName,
                                     _source=l_fields, stored_fields=l_fields, sort=['timestamp_ms:asc'],
                                     from_=nFrom, size=nSize, body={"query":{"match":{self.sTriName:{"query": 1}}}})
    ...
    def get_nok(self, nFrom=0, nSize=2200):
        nIndexSize = int(self.xEs.count(index=self.sIndexName,
                                        body={"query": {"match": {self.sTriName: {"query": -1}}}})['count'])

        l_fields = ['timestamp_ms', 'id_str', 'text', 'user.id_str', 'has_image']

        x_response = self.xEs.search(index=self.sIndexName, doc_type=self.sDocTypeName, _source=l_fields,
                                     stored_fields=l_fields, sort=['timestamp_ms:asc'], from_=nFrom, size=nSize,
                                     body={"query":{"match":{self.sTriName:{"query": -1}}}})
    ...
    def search_retweet(self, s_tweet_id):
        ...
        data = self._retweet_from_scroll(x_response, len(t_words))
    ...
    def _retweet_from_scroll(self, x_response, nb_words):
        ...
        n_cmpt += 1
        nb_response = int(x_response['hits']['total'])

        while n_cmpt < nb_response and n_cmpt < int(self.sLimit):
            try:
                n_cmpt -= 1
    ...
    def _return_from_scroll(self, x_response, nSize=2200):
        n_cmpt = 0
        nFrom = 0
        data = []
        ...
        for hit in x_response['hits']['hits']:
            st = datetime.datetime.fromtimestamp(int(hit['_source']['timestamp_ms'])/1000).strftime('%Y-%m-%d %H:%M:%S')
            data.append({"date": str(st), "user": str(hit['_source']['user']['id_str']),
                         "id": str(hit['_source']['id_str']), "text": str(hit['_source']['text'])})
            n_cmpt += 1
            self.xLastResult = hit['_source']['timestamp_ms']
        n_cmpt += 1
        jsonfile = {"draw": 0, "data": data, "recordsTotal": n_cmpt, "recordsFiltered": n_cmpt}
